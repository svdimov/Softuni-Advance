...
class RobotsManagingApp:
    ...
    def add_service(self, _type: str, name: str):
        if _type not in self.service_type:
            raise Exception("Invalid service type!")
        service = self.service_type[_type](name)
        self.services.append(service)
        return f"{_type} is successfully added."

    def add_robot(self, _type: str, name: str, kind: str, price: float):
        if _type not in self.robots_type:
            raise Exception("Invalid robot type!")
        robot = self.robots_type[_type](name, kind, price)
    ...
    def feed_all_robots_from_service(self, service_name: str):
        ...
        sad = [r.eating() for r in service.robots]
        return f"Robots fed: {len(sad)}."
